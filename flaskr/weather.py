...
def add_city(latitude, longitude):
    ...
    if r.status_code != 200:
        ...
        if city is None:
            ...
            db.session.add(city)
            db.session.commit()
        from .tasks import add_weather_task
        add_weather_task(city.city_id)
        return city.city_id
    ...
    if filename is None:
        ...
        with open(filename) as csv_file:
            ...
            for line in data:
                ...
                entry = db.session.query(OwmCurrentWeather).filter_by(timestamp=timestamp, city_id=city_id).first()
            ...
            db.session.commit()
            if line_skip>0:
                error = 'skiped lines: ' +str(line_skip)
    return error
...
def current_forecast(city_id):
    ...
    data = db.session.query(OwmHourlyWeatherForecast) \
        .filter_by(city_id=city_id) \
        .order_by(OwmHourlyWeatherForecast.forecast_timestamp.desc(), OwmHourlyWeatherForecast.request_timestamp.desc()) \
        .all()
    #.order_by(OwmHourlyWeatherForecast.forecast_timestamp.asc(), OwmHourlyWeatherForecast.request_timestamp.desc()).first())
...
def current_weather_dump(city_id):
    ...
    data = OwmCurrentWeather.query \
        .filter_by(city_id=city_id) \
        .order_by(OwmCurrentWeather.timestamp.asc()).all()

    weather = {"city": row2dict(city), "count": len(data), "data": []}
    for d in data:
        entry = row2dict(d)
        entry.pop('id')
        entry.pop('city_id')
...
def remove_city(city_id):
    ...
    if city is None:
        abort(404, f"City id {city_id} doesn't exist.")
    db.session.query(OwmCurrentWeather).filter(OwmCurrentWeather.city_id==city_id).delete()
...
def get_weather_by_date():
    ...
    if data is None:
        ...
        # find all data withing range and pick the one with most recent request time
